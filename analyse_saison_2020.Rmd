...
subtitle: "Was kann man aus 220 Jollen-Meilen auf der Alster lernen? (Oder: Was macht ein segelnder Data Scientist eigentlich im Herbst?)"
author: Sandro Raabe
...
draw_leaflet <- function(col_var){
  ...
  for (i in seq_along(lons)){
    base <- base %>% addPolylines(lons[[i]], lats[[i]], color = cols[i], group = groups[i], weight = 1.5, opacity = 0.7)
  }
...
* Außer den `r anz` Seemeilen auf der Alster bin ich im Jahr 2020 noch 280 Seemeilen auf der Ostsee unterwegs gewesen - diese sind hier außen vor, wären allerdings eine eigene Analyse wert.
