---
title: "Was kann man aus 175 Seemeilen auf der Alster lernen? (oder: Was macht ein segelnder Data Scientist eigentlich im Herbst?)"
author: Sandro Raabe
...
* Segelpartner **Erik und Leo** folgt mir bis zu den **hintersten Brücken**, **Felix** bis zur **Alsterperle**.
...
* Gesamtstrecke aufgezeichnet: **173 Seemeilen**, entspricht etwa `r round(173*1.8)` km
...
ggplot(dfr,aes(x=week,y=day))+
  ...
  scale_fill_manual(values=c("green","lightgrey","#fb8072","#d3d3d3"))+
...
> Beobachtung: Mittwochs scheint mir die große Känguruhregatta das Segeln zu vermiesen, bei der vereinsinternen Dienstagsregatta scheint ich jedoch gerne dabei zu sein. (Oder: Nach der Dienstagsregatta bin ich so durch, dass Mittwochs gar nichts geht?)
...
draw_map <- function(col_var, nrow){
  ...
  ggmap(alster_map) + geom_point(data = dat,
...
# TODO: change levels to "Alle Montage"
...
Zu jedem GPS-Punkt ist die momentane Peilung bekannt (die "Fahrtrichtung"). Dies können wir auszählen und als Histogramm (Häufigkeitsdiagramm) wie eine Kompassrose darstellen:
...
all_dat %>% mutate(bin = cut(course, breaks = seq(-1,361, 3))) %>% 
  ...
  hc_chart(polar = TRUE) %>% 
  hc_title(text = "Kompasspeilungen nach Häufigkeit") %>% 
...
> Beobachtung: Der übliche Start-Weg vom OSG-Steg geht nach Nord-Nordost
...
hchart(dat, type = "treemap", 
  ...
  hc_title(text = glue("Segelpartner - Größe entspricht Anzahl Sessions"))
```
...
# Anmerkungen, Limiterungen und Ausblick

* nicht alle Tracks habe ich mitgeschnitten (nur schätzungsweise 80%)
* außer den 175 Seemeilen auf der Alster bin ich im Jahr 2020 noch 280 Seemeilen auf der Ostsee unterwegs gewesen - diese sind hier außen vor, wären allerdings eine eigene Analyse wert
* ...

# Credits

Danke OSG für die tolle Gemeinschaft, die tollen Boote und die tollen Angebote!
