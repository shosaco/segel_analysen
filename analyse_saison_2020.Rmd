...
subtitle: "Was kann man aus 220 Jollen-Meilen auf der Alster lernen?<br/>(Oder: Ach das macht ein segelnder Data Scientist also im Herbst!)"
author: Sandro Raabe
...
```{r include=FALSE}
knitr::opts_chunk$set(echo = FALSE, warning=FALSE, message=FALSE, cache = TRUE, cache.comments = FALSE)
...
draw_map <- function(col_var, nrow, colorize=FALSE){
  ...
  if (col_var == "Wochentag"){
    out <- out + facet_wrap(col_var, nrow = nrow, 
                            labeller = as_labeller(setNames(unique(glue("Alle {t}e", t = all_dat[[col_var]])), unique(all_dat[[col_var]]))))
  }else{
    out <- out +
...
pirateplot(formula = speed ~ Boot,# point.col = "#6B6B6B",
           data = dat, inf.f.o = 0.1, point.o = .005) 
```

Interessant ist hier: Der Kielzugvogel war im Durchschnitt (Bauch der Verteilung) der schnellste, im Median (fette Linie) gewinnt jedoch die 470er - dies liegt an den Ausreißern in der Maximalgeschwindigkeit, die wir mit dem Zugvogel hatten (der Median kümmert sich nicht um Ausreißer, der Durchschnitt jedoch schon).

**Beobachtung:** Interessanterweise sind die Gewichts-Extreme auch die schnellsten: Die leichte Gleitjolle *470er* und die schweren Kielboote *Kielzugvogel* und *C55*, wobei der Kielzugvogel deutlich die größte Maximalgeschwindigkeit erreicht. Die *J70* ist das Bundesliga-Regatta-Boot und sollte schneller sein, aber an dem einen Tag war wohl nicht viel Wind :-)
